<template>
  <div>
    <form style="margin: 3px" v-if="checkEdit('staffratio')">
      <NumberBox
        placeholder="Поз."
        :min="1"
    ...
    <div style="height: 490px">
      ...
      >
        ...
        <GridColumn field="title" title="Наименование" width="230"></GridColumn>
        <GridColumn field="quantity" title="Кол-во" :editable="true" width="70">
          <template slot="edit" slot-scope="scope">
            <NumberBox v-model="scope.row.quantity" :precision="0" decimalSeparator=","></NumberBox>
          </template>
        </GridColumn>
        <GridColumn field="ratio" title="К.Зап." :editable="true" width="65">
          <template slot="edit" slot-scope="scope">
            <NumberBox v-model="scope.row.ratio" :precision="3" decimalSeparator=","></NumberBox>
          </template>
        </GridColumn>
    ...
    <div class="buttons_bar" v-if="checkEdit('staffratio')">
...
export default {
  ...
  data() {
    return {
      ...
      part_types: [], // Список типов для подстановки
  ...
  methods: {
    ...
    getStaff() {
      // Получение состава
      const url = "partlink/";
      const params = { parent: this.$store.getters.getSelectedId };
    ...
    onRenumClick() {
      ...
      let start_row = { parent: this.$store.getters.getSelectedId }; // Параметры строки для начала перенумерации
      for (const q of this.staff_data.filter((row) => row.checked)) {
        start_row.pk = q.pk;
